...
class KazakhLemmatizer(BaseLemmatizer):
    def __init__(self, language='kazakh'):
        super().__init__(language)
        current_dir = os.path.dirname(os.path.abspath(__file__))
        self.python_venv_path = os.path.join(current_dir, ".venv", "Scripts", "python.exe")
        self.script_path = os.path.join(current_dir, "kazakh_subprocess.py")
    ...
    def get_lemma_counts(self, text):
        ...
        #     raise RuntimeError(f"Kazakh subprocess failed: {result.stderr}")
...
if __name__ == "__main__":
    ...
    text = "Бұл қазақ тілінде жазылған сөйлем."
